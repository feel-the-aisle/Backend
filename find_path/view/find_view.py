...
from find_path.service.find_service import Detectservice

# Blueprint 객체 생성
findpath_bp = Blueprint('findpath_bp', __name__)

# 라우트 등록하기
@findpath_bp.route('/find_shortest_path', methods=['POST'])
def find_shortest_path():
          ...
          result = Detectservice.find_shortest_path(storeName, startP, endP)
        #   # path 값 = 연산 결과 예시
...
def jindongMap_result(storeName, startP, endP):
          ...
          # 지도 만들기
          maze = Detectservice.create_maze(storeName, galo, selo)

          # 출발, 도착 지점 가져오기
          points = Detectservice.get_start_end(startP, endP, selo, galo, maze)
          # [0]: 출발 가로 / [1]: 출발 세로 / [2]: 도착 가로 / [3]: 도착 세로
          start = (points[1], points[0])
          end = (points[3], points[2])

          # 경로 찾기
          path = Detectservice.find_shortest_path(maze, start, end)
          return path
